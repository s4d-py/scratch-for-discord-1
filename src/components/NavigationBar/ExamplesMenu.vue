    ...
    mounted() {
        setTimeout(() => {
            ...
            if (urlParams.has('exampleid')) {
                fetch(`https://s4dpyexamplestest.frostzzone.repl.co/api/getExample?id=${urlParams.get("exampleid")}`)
                    .then(async (result) => {
                        result.json().then((json) => {
                            const lkjgenwhikgu4ewkjn = document.createElement('div');
                            displaySwalPopupForUserExample(json, lkjgenwhikgu4ewkjn, urlParams.get("exampleid"), "https://s4dpyexamplestest.frostzzone.repl.co/", this.$swal, this.$store.state.workspace, this.$toast)
                        })
                    })
            }
        }, 500);
